<template>
  <div v-if="quoteCartOpen" class="cart-backdrop" @click.self="closeQuoteCart">
    <aside class="cart-drawer">
      ...
      <h2>Cantidad: <span>{{ quoteItemCount }}</span></h2>
      <div v-if="quoteItems.length" class="cart-list">
        <div v-for="item in quoteItems" :key="item.quoteKey || `${item.name}-${item.details || ''}`" class="cart-item">
          <div><strong>{{ item.name }}</strong><small>{{ formatPrice(item.price) }} c/u</small><small v-if="item.details">{{ item.details }}</small></div>
          <div class="quantity"><button aria-label="Reducir cantidad" @click="updateQuoteQuantity(item.name, -1, item.details, item.quoteKey)"><Minus :size="13" /></button><span>{{ item.quantity }}</span><button aria-label="Aumentar cantidad" @click="updateQuoteQuantity(item.name, 1, item.details, item.quoteKey)"><Plus :size="13" /></button></div>
        </div>
      </div>
