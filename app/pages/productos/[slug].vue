    ...
    <main class="detail-main">
      <div class="detail-layout">
        ...
        <section class="product-summary">
          ...
          <h1>{{ product.name }}</h1>
          <p class="price-range" aria-live="polite">{{ isRack ? (quote.ready ? formatPrice(quote.price) + " + IVA" : "Configuración pendiente") : formatRange(product) }}</p>
          <div v-if="isRack" class="rack-config">
            <p class="eyebrow">Cotización de rack</p><p class="config-copy">Elegí una medida para actualizar el precio estimado en tiempo real.</p>
            <p v-if="rackError" class="config-copy" role="alert">No pudimos cargar las opciones del rack. Recargá la página para volver a intentarlo.</p>
            <div v-else class="calculator">
              <div class="calculator-head"><span>Configuración</span><span>REF. 2026</span></div>
              <div class="select-row"><label v-for="group in rackOptionGroups" :key="group.key">{{ group.label }}<select v-model="selection[group.key]"><option disabled value="">Sin opción inicial</option><option v-for="option in activeRackOptions(settings,group.key)" :key="option.id" :value="option.id">{{ option.value }} {{ group.unit }}</option></select></label></div>
              <p v-if="!quote.ready" class="config-copy" role="status">Los precios de esta configuración todavía no están disponibles.</p>
              <div class="estimate-result" aria-live="polite"><span>Estimado + IVA</span><strong>{{ quote.ready ? formatPrice(quote.price) : 'Configuración pendiente' }}</strong></div>
            </div>
          </div>
          <div class="purchase-actions"><div class="quantity"><button aria-label="Reducir cantidad" @click="quantity = Math.max(1, quantity - 1)"><Minus :size="15" /></button><span>{{ quantity }}</span><button aria-label="Aumentar cantidad" @click="quantity++"><Plus :size="15" /></button></div><button class="button button-primary" :disabled="isRack && !quote.ready" @click="addProduct">Agregar al carrito <ShoppingCart :size="17" /></button><button class="button button-dark" :disabled="isRack && !quote.ready" @click="requestQuote">Comprar ahora <ArrowUpRight :size="17" /></button></div>
        </section>
      </div>
      <section class="description-section">
        <h2>Descripción:</h2><p class="product-text">{{ product.technicalDescription }}</p>
        <template v-if="product.specifications?.trim()">
          <h2>Especificaciones</h2><p class="product-text">{{ product.specifications }}</p>
        </template>
      </section>
    </main>
  </div>
...
import { defaultRackCalculator, settingsForRack, activeRackOptions, calculateRackPrice, rackQuoteItem, rackOptionGroups, emptyRackSelection, type RackCalculatorConfig, type RackQuote } from "../../../data/rack-calculator";
const { data: rackData, error: rackError } = await useFetch<RackCalculatorConfig>("/api/rack-calculator", { key: "rack-calculator" });
const route = useRoute();
const { data: productData } = await useFetch<CatalogProduct[]>("/api/products", { key: "catalog-products" });
...
const settings = computed(() => settingsForRack(rackData.value || defaultRackCalculator(), product.value));
const selection = ref(emptyRackSelection());
watch([() => product.value?.id, settings], ([id, current], previous) => {
  for (const { key } of rackOptionGroups) {
    const stillAvailable = activeRackOptions(current,key).some(option => option.id === selection.value[key]);
    if (id !== previous?.[0] || !stillAvailable) selection.value[key] = current.defaultOptionIds[key];
  }
}, { immediate: true });
const quote = computed<RackQuote>(() => rackError.value || !rackData.value ? { ready: false, reason: "Configuración no disponible." } : calculateRackPrice(product.value, settings.value, selection.value));
